script({
  ...
  parameters: {
    ...
    videoUrl: {
      type: "string",
      description:
        "Direct video URL to analyze (alternative to extracting from issue body)",
    },
    localFiles: {
      type: "string",
      description:
        "Local directory path to scan for *.mp4 files, or specific file path to a *.mp4 file",
    },
  },
});
...
const { instructions, videoUrl, localFiles } = vars as {
  instructions?: string;
  videoUrl?: string;
  localFiles?: string;
};
...
async function processLocalFiles(localFilesPath: string) {
  ...
  let stat;
  try {
    stat = await fs.promises.stat(localFilesPath);
  } catch (error) {
    output.error(
      `Cannot access path: ${localFilesPath}. Error: ${error.message}`,
    );
    return;
  }
  ...
  if (stat.isFile()) {
    ...
    let files;
    try {
      files = await fs.promises.readdir(localFilesPath);
    } catch (error) {
      output.error(
        `Cannot read directory: ${localFilesPath}. Error: ${error.message}`,
      );
      return;
    }
    ...
    output.error(
      `Local files path ${localFilesPath} is neither a file nor a directory`,
    );
  }
}
...
async function processLocalVideoFile(videoPath: string) {
  ...
  try {
    ...
    output.error(
      `Cannot access video file: ${videoPath}. Error: ${error.message}`,
    );
    return;
  }
  ...
  try {
    ...
    output.error(`Failed to process video ${videoPath}: ${error.message}`);
  }
}
